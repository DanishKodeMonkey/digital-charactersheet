...
function Character() {
  ...
  const removeFeat = (index) =>
    setFeats((prev) => prev.filter((item) => item.index !== index));

  const addSpecialAbility = (SpecialAbility) =>
    setSpecialAbilities((prev) => [...prev, SpecialAbility]);
  const removeSpecialAbility = (index) =>
    setSpecialAbilities((prev) => prev.filter((item) => item.index !== index));
